pipeline{
    ...
    stages{
        ...
        stage('Build Docker Image'){
            steps{
                bat "docker build -t ${DOCKER_REPO_NAME}/i-${USERNAME}-feature:${BUILD_NUMBER} -t ${DOCKER_REPO_NAME}/i-${USERNAME}-feature:latest ."
            }
        }
        ...
        stage('Publish to DCR'){
            steps{
                withDockerRegistry([credentialsId: 'DockerHub', url: ""]){
                    bat "docker push ${DOCKER_REPO_NAME}/i-${USERNAME}-feature:${BUILD_NUMBER}"
                    bat "docker push ${DOCKER_REPO_NAME}/i-${USERNAME}-feature:latest "
                }
            }
        }
                ...
                stage('Docker Deployment'){
                    steps{
                        script{
                            ...
                            containerIdCheck = "${bat (script: "docker ps -a -q -f status=running -f name=c-${USERNAME}-feature", returnStdout: true).trim().readLines().drop(1).join("")}"
                            if(containerIdCheck != ''){
                            ...
                            bat "docker run --name c-${USERNAME}-feature -d -p ${DOCKER_FEATURE_PORT}:${APP_PORT} ${DOCKER_REPO_NAME}/i-${USERNAME}-feature:latest"
                        }
                    }
                }
